use super::{
    CompetitionStage, ContinentalMatch, ContinentalMatchweek, EUROPA_LEAGUE_ID, GroupTable,
    KnockoutTie,
};
    ...
    fn every_fixture_is_played_on_the_competitions_night_in_any_year() {
        for year in [2026, 2027, 2028] {
            ...
            for m in &comp.matches {
                assert!(
                    EuropaLeague::MATCH_WEEKDAYS.contains(&m.date.weekday()),
                    "{year}: {:?} fixture on a {}",
                    m.stage,
                    m.date.weekday()
                );
