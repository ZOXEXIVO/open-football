...
impl PlayerValueCalculator {
    ...
    fn determine_career_consistency_factor(player: &Player) -> f64 {
        ...
        let weighted_rating: f64 = rated_seasons
            .iter()
            .map(|h| h.statistics.average_rating_realistic(pos) as f64 * h.statistics.played as f64)
            .sum::<f64>()
            / total_games as f64;
...
mod tests {
    ...
    fn round_market_value_rounds_correctly() {
        assert_eq!(
            PlayerValueCalculator::round_market_value(15_432_100.0),
            15_000_000.0
        );
        assert_eq!(
            PlayerValueCalculator::round_market_value(1_567_000.0),
            1_600_000.0
        );
        assert_eq!(
            PlayerValueCalculator::round_market_value(237_000.0),
            250_000.0
        );
        assert_eq!(
            PlayerValueCalculator::round_market_value(43_000.0),
            40_000.0
        );
