...
impl Player {
    ...
    fn process_injury(&mut self, result: &mut PlayerResult, now: NaiveDate) {
        ...
        if self.player_attributes.is_injured {
            ...
            if rand::random::<f32>() < 0.001 * proneness_modifier {
                if let Some(body_part) = crate::club::player::injury::BodyPart::from_u8(
    ...
    fn injury_for_body_part(body_part: crate::club::player::injury::BodyPart) -> InjuryType {
        use crate::club::player::injury::BodyPart;
        match body_part {
