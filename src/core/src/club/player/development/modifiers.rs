...
/// Namespace for the per-tick multipliers that scale base growth/decline
/// rates. All functions are pure — they take the relevant attributes and
/// return a multiplicative factor.
pub(super) struct DevelopmentModifiers;

impl DevelopmentModifiers {
    ...
    // them into a multiplier capped at 1.40. Replaced in the development
    // tick by [`super::maturity::senior_exposure_multiplier`], which reads
    // rolling minutes and physical load directly off `PlayerLoad` instead
    // of raw appearance counts. The function below is retained for any
    // downstream consumer that still wants the count-based summary view.
    #[allow(dead_code)]
