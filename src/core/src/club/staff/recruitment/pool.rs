...
/// The pool of staff between jobs, and its weekly clock.
///
/// A sacked or expired coach lands here and is drawn from here; the
/// tick ages the pool and harvests anyone who has waited long enough
/// to retire out of it.
...
impl StaffPool {
    ...
    /// pool bookkeeping.
    pub(super) fn entry_date(staff: &Staff) -> Option<NaiveDate> {
        staff.recent_performance.last_evaluation_date
    }

    pub(super) fn set_entry_date(staff: &mut Staff, date: NaiveDate) {
    ...
    pub fn admit(pool: &mut Vec<Staff>, mut staff: Staff, today: NaiveDate) {
        staff.contract = None;
        ...
        Self::set_entry_date(&mut staff, today);
        debug!(
    ...
    pub fn tick(pool: &mut Vec<Staff>, today: NaiveDate) {
        pool.retain_mut(|staff| {
            ...
            if age >= 65 {
                let years_unsigned = Self::entry_date(staff)
    ...
    /// staff member.
    pub fn harvest_expired(data: &mut SimulatorData, today: NaiveDate) -> usize {
        ...
        for staff in harvested {
            StaffPool::admit(&mut data.free_agent_staff, staff, today);
        }
        n
    }
}
...
mod tests {
    ...
    fn admit_clears_contract_and_relations() {
        ...
        StaffPool::admit(&mut pool, staff, today);

        assert_eq!(pool.len(), 1);
        assert!(pool[0].contract.is_none());
        assert_eq!(pool[0].fatigue, 0.0);
        assert_eq!(StaffPool::entry_date(&pool[0]), Some(today));
    }
    ...
    fn pool_tick_soft_retires_unemployed_seniors() {
        ...
        StaffPool::set_entry_date(&mut senior, admit_day);
