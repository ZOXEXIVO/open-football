...
impl CoachDecisionState {
    ...
    pub(super) fn lens_skill_composite(&self, player: &Player) -> f32 {
        ...
        tech_score * lens.technical_weight
    ...
    pub(super) fn compute_visibility(&self, player: &Player, team_type: &TeamType) -> f32 {
        let base = match team_type {
            ...
            | TeamType::U23
            | TeamType::U20
            | TeamType::U21 => 0.5 + self.profile.youth_preference * 0.15,
            TeamType::U18 | TeamType::U19 => 0.2 + self.profile.youth_preference * 0.3,
        };
    ...
    pub fn perceived_quality(&self, player: &Player, date: NaiveDate) -> f32 {
        ...
        let (
            ...
            .map(|imp| {
                (
                    imp.bias.quality_offset,
                    imp.bias.visibility,
                    imp.bias.first_impression,
                    imp.bias.anchored,
                    imp.bias.perception_drift,
                    imp.bias.overreaction_magnitude,
    ...
    pub fn match_readiness(&self, player: &Player) -> f32 {
        ...
        let noise = self
            .profile
            .perception_noise(player.id, self.current_week.wrapping_add(0xFE57))
            * noise_scale;

        let physicality_illusion = if intuition < 0.5 {
            let physical_look =
                (player.skills.physical.strength + player.skills.physical.pace) / 40.0;
    ...
    pub fn potential_impression(&self, player: &Player, date: NaiveDate) -> f32 {
        ...
        let noise = self
            .profile
            .perception_noise(player.id, week.wrapping_add(7919))
            * noise_scale
            * 3.0;
        ...
        let pace_bonus = ((player.skills.physical.pace - 12.0).max(0.0) / 8.0)
            * self.profile.physical_bias_youth
            * 1.5;
    ...
    pub fn training_impression(&self, player: &Player) -> f32 {
        ...
        let noise = self
            .profile
            .perception_noise(player.id, self.current_week.wrapping_add(42))
            * noise_scale
            * 1.5;
