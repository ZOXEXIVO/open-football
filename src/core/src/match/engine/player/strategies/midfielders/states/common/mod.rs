...
#[derive(Default)]
pub struct MidfielderCommonState {}

impl StateProcessingHandler for MidfielderCommonState {
...
pub enum ActivityIntensity {
    /// Very high intensity - explosive actions (tackling, shooting, distance shooting, heading)
    VeryHigh,
    /// High intensity - sustained running, pressing, intercepting, attacking support
    High,
    /// Moderate intensity - dribbling, distributing, tracking runner, returning, creating space
    Moderate,
    /// Low intensity - walking, passing, holding possession, switching play
    Low,
    /// Recovery - standing still, resting with minimal movement
    Recovery,
}
...
impl ActivityIntensity {
    ...
    pub fn base_fatigue(&self) -> f32 {
        match self {
            ActivityIntensity::VeryHigh => 8.0,  // Explosive actions tire quickly
            ActivityIntensity::High => 5.0,      // Base from running state
...
/// Midfielder condition processor
pub struct MidfielderCondition {
    intensity: ActivityIntensity,
    use_velocity: bool,
}

impl MidfielderCondition {
    ...
    pub fn process(self, ctx: ConditionContext) {
        ...
        let intensity_multiplier = if self.use_velocity {
            ...
            // Intensity multiplier: walking (0.3x), jogging (0.6x), running (1.0x), sprinting (1.5x)
