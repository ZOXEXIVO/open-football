...
/// 1v1 duel resolution machinery. Zero-sized struct so the module
/// exposes no free functions — call sites read
/// `DribbleDuelResolver::resolve(..)`.
pub struct DribbleDuelResolver;

impl DribbleDuelResolver {
    ...
    fn attacker_score(attacker: &MatchPlayer, ctx: DuelContext) -> f32 {
        ...
        score += Self::attacker_trait_bonus(attacker, ctx);
        score
    }
    ...
    fn defender_score(defender: &MatchPlayer, ctx: DuelContext) -> f32 {
        ...
        score += Self::defender_trait_bonus(defender, ctx);
        score
    }
...
mod tests {
    ...
    fn elite_dribbler_beats_average_defender_more_often() {
        ...
        for i in 0..200 {
            ...
            let r = DribbleDuelResolver::resolve(&elite, &avg_def, DuelContext::default(), roll);
    ...
    fn second_defender_cover_helps_defenders() {
        ...
        for i in 0..200 {
            let roll = (i as f32 + 0.5) / 200.0;
            let r1 = DribbleDuelResolver::resolve(&attacker, &defender, solo, roll);
            let r2 = DribbleDuelResolver::resolve(&attacker, &defender, cover, roll);
