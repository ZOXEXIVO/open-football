//! **The tick's sub-phases, and the position recorder** — the three
//! calls `game_tick_inner` makes to move the world on (ball, outfield
//! players, keepers), the level-of-detail rule that lets far-from-the-ball
//! players think on a slower cadence, and the sampler that writes the
//! replay's position track.
...
impl<const W: usize, const H: usize> FootballEngine<W, H> {
    ...
    pub(in crate::r#match::engine::engine) const POSITION_RECORD_INTERVAL_MS: u64 = 30;
    ...
    pub(in crate::r#match::engine::engine) fn play_ball(
        field: &mut MatchField,
    ...
    pub(in crate::r#match::engine::engine) fn play_players(
        field: &mut MatchField,
        context: &mut MatchContext,
        tick_context: &GameTickContext,
        events: &mut EventCollection,
    ...
    pub(in crate::r#match::engine::engine) fn play_goalkeepers(
        field: &mut MatchField,
        context: &mut MatchContext,
        tick_context: &GameTickContext,
        events: &mut EventCollection,
