...
use self::depth::EmergencyDepthRequestIntent;
use self::market::FreeAgentMarketPass;
pub(in crate::country::result) use self::market::{FreeAgentLedger, FreeAgentWorld};
...
#[cfg(test)]
use self::depth::EmergencyDepthRequestPlanner;
#[cfg(test)]
use super::types::TransferActivitySummary;
#[cfg(test)]
...
impl FreeAgentPass {
    ...
    pub(in crate::country::result) fn handle_free_agents(
        country: &mut Country,
        date: NaiveDate,
        world: &FreeAgentWorld<'_>,
        ledger: &mut FreeAgentLedger<'_>,
    ) -> Vec<GlobalFreeAgentSigning> {
        FreeAgentMarketPass::run(country, date, world, ledger)
    }
