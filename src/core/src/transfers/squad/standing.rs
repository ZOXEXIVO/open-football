...
//! not OUTPUT" channel.
//!
//! [`crate::transfers::scouting::breakout::BreakoutPerformanceSignal`] can only read a scoreline:
...
#[derive(Debug, Clone, Copy)]
pub(in crate::transfers) struct StandingInputs {
...
#[derive(Debug, Clone, Copy)]
pub(in crate::transfers) struct StandingSignal {
    pub score: f32,
}
...
impl StandingSignal {
    ...
    pub(in crate::transfers) fn neutral_rating(group: PlayerFieldPositionGroup) -> f32 {
        match group {
    ...
    pub(in crate::transfers) fn compute(inp: &StandingInputs) -> StandingSignal {
