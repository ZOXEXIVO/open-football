//! What the club needs, and what it will part with.
//!
//! The squad review that turns a roster into transfer requests lives in
//! this file. [`plan`] writes the brief it recruits against, [`ledger`]
//! prices who it would sell, [`needs`] is the emergency floor a squad may
...
pub use needs::*;
pub use plan::*;
pub use standing::CareerRecordSnapshot;
...
use crate::transfers::loan::home::{SquadHomeContext, UnsettledAbroadScan};
...
use crate::{
    Club, ClubPhilosophy, Country, LoanSpellVerdict, MatchTacticType, Player,
    PlayerFieldPositionGroup, PlayerPlanRole, PlayerPositionType, PlayerStatusType,
    ReputationLevel, TACTICS_POSITIONS,
};
...
struct SquadEvaluation {
    ...
    /// [`crate::transfers::squad::plan::SquadPlanner`]. The requests
    ...
    /// [`crate::transfers::squad::ledger::AssetLedger`]. Marketed,
    /// never listed.
    sell_list: Vec<SellListEntry>,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(in crate::transfers) enum NeedKind {
    FormationGap,
    QualityUpgrade,
    DepthCover,
}
...
pub(in crate::transfers) struct GroupNeed {
    pub group: PlayerFieldPositionGroup,
    pub representative_pos: PlayerPositionType,
    pub kind: NeedKind,
}

/// How a squad's shape turns into recruitment needs.
///
/// Pure functions of the formation and the roster — no side effects,
/// no world access — so evaluation and tests share one source of truth.
pub(in crate::transfers) struct GroupNeedScan;
...
impl GroupNeedScan {
    ...
    /// one source of truth.
    pub(in crate::transfers) fn depth_requirement(
    ...
    pub(in crate::transfers) fn needs(
        ...
        for (pos, _, _) in position_coverage {
            ...
            let group_count = squad
                ...
                .count();
            if group_count < GroupNeedScan::depth_requirement(formation_positions, group) {
                needs.push(GroupNeed {
                    group,
                    representative_pos,
...
#[derive(Debug, Clone, Copy)]
pub(in crate::transfers) struct InvestmentAppetite {
...
impl InvestmentAppetite {
    ...
    pub(in crate::transfers) fn of(club: &Club, rep_score: f32, date: NaiveDate) -> Self {
        let standing =
    ...
    pub(in crate::transfers) fn is_active(&self) -> bool {
        self.score > 0.0
    }
}
...
pub(in crate::transfers) struct InvestmentTarget {
    pub group: PlayerFieldPositionGroup,
    pub representative_pos: PlayerPositionType,
...
pub(in crate::transfers) struct InvestmentWatch {
...
pub(in crate::transfers) struct SuccessionAudit;
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(in crate::transfers) enum SuccessionUrgency {
...
impl SuccessionAudit {
    ...
    pub(in crate::transfers) fn career_end_age(group: PlayerFieldPositionGroup) -> u8 {
        match group {
    ...
    /// PA.
    pub(in crate::transfers) fn heir_in_place(
        squad: &[SquadPlayerInfo],
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(in crate::transfers) enum PathwayAction {
...
#[derive(Debug, Clone, Copy)]
pub(in crate::transfers) struct ProspectSignals {
    pub age: u8,
...
pub(in crate::transfers) struct ProspectPathway;
...
impl PipelineProcessor {
    ...
    pub(in crate::transfers) const MAX_INVESTMENT_SHARE: f64 = BriefTier::A_SHARE;
    ...
    fn evaluate_single_club(
        ...
        match SquadReview::open(club, date, current_window, mid_season_window, home) {
            SquadReviewOpening::Review(review) => review.run(),
            SquadReviewOpening::Bootstrap(evaluation) => evaluation,
        }
    }
