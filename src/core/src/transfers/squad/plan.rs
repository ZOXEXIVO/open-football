...
/// planner's appetite and the input to [`crate::transfers::value::upgrade::MoneyUtility`]
/// on the buying side.
...
impl SquadPlan {
    ...
    pub(in crate::transfers) fn build(inputs: &PlanInputs<'_>) -> Self {
        ...
        for pos in inputs.formation_positions.iter() {
            let group = pos.position_group();
            depth_targets.entry(group).or_insert_with(|| {
                GroupNeedScan::depth_requirement(inputs.formation_positions, group)
            });
        }
...
/// walk of the roster.
pub(in crate::transfers) struct PlanInputs<'a> {
    pub club: &'a Club,
...
impl SquadPlanner {
    ...
    pub(in crate::transfers) fn plan(inputs: &PlanInputs<'_>) -> RecruitmentBrief {
    ...
    fn brief_depth(
        ...
        for group in [
            ...
            // `GroupNeedScan::needs` picks the specific uncovered slot, so a
