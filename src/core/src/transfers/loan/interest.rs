...
pub(in crate::transfers) struct ClubOpinion {
...
impl ClubOpinion {
    ...
    pub(in crate::transfers) fn of(club_id: u32, player_id: u32, judging: u8) -> Self {
    ...
    pub(in crate::transfers) fn believed_ability(&self, truth: u8) -> f32 {
...
/// name.
pub(in crate::transfers) struct BorrowerTaste {
    club_id: u32,
...
impl BorrowerTaste {
    ...
    /// preference only.
    pub(in crate::transfers) fn interest_in(
        &self,
...
pub(in crate::transfers) struct LoanCandidateProfile {
    pub player_id: u32,
...
/// parked there.
pub(in crate::transfers) struct DestinationAppeal {
...
impl DestinationAppeal {
    ...
    pub(in crate::transfers) fn score(&self) -> f32 {
...
/// out forever.
pub(in crate::transfers) struct InterestDraw;
...
impl InterestDraw {
    ...
    /// foregone conclusion.
    pub(in crate::transfers) const SHARPNESS: f32 = 3.0;

    /// Draw one `(id, interest)` pair. `None` for an empty slate.
    pub(in crate::transfers) fn pick(options: &[(u32, f32)]) -> Option<u32> {
        Self::pick_index(options).map(|i| options[i].0)
    }

    /// Draw one option, returning its index — for callers whose payload is
    /// richer than an id.
    pub(in crate::transfers) fn pick_index(options: &[(u32, f32)]) -> Option<usize> {
        match options.len() {
            0 => return None,
            1 => return Some(0),
            _ => {}
    ...
    /// up in a world tick.
    pub(in crate::transfers) fn pick_several(options: &[(u32, f32)], count: usize) -> Vec<u32> {
    ...
    pub(in crate::transfers) fn visit_order(len: usize) -> Vec<usize> {
        if len <= 1 {
...
impl LoanApproachMemory {
    ...
    pub(in crate::transfers) const FORGET_DAYS: i64 = 120;
    ...
    pub(in crate::transfers) const PLACEMENT_MEMORY_DAYS: i64 = 300;
    ...
    pub(in crate::transfers) fn rebuff_days(approaches: u8) -> i64 {
    ...
    pub(in crate::transfers) fn crowding_at(
...
/// make.
pub(in crate::transfers) struct GroupPressure;

impl GroupPressure {
    pub(in crate::transfers) fn thinness(headcount: usize, group: PlayerFieldPositionGroup) -> f32 {
