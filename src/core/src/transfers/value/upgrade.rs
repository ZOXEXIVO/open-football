...
use crate::transfers::squad::plan::{BriefTier, MoneySlack, RoleWeight};
...
#[derive(Debug, Clone, Copy)]
pub(in crate::transfers) struct TargetBelief {
    pub group: PlayerFieldPositionGroup,
    pub tier: BriefTier,
    pub believed_level: f32,
    pub incumbent_level: f32,
...
impl UpgradeMath {
    ...
    pub(in crate::transfers) fn for_club(
        club: &Club,
        date: NaiveDate,
        target: &TargetBelief,
    ) -> DealValue {
    ...
    pub(in crate::transfers) fn priced(
        club: &Club,
        date: NaiveDate,
        target: &TargetBelief,
    ) -> Option<DealValue> {
