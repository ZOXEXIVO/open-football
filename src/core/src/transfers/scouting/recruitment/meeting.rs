        ...
        for p in pending {
            if let Some(club) = country.clubs.iter_mut().find(|c| c.id == p.club_id) {
                ...
                for promo in p.promotions {
                    MeetingOutcome::apply_promotion(plan, promo);
                }
...
/// What a recruitment meeting does with its own conclusions: how a
/// scout's monitoring row becomes a vote, and how a promoted player
...
mod tests {
    ...
    fn strong_fit_high_confidence_yields_strong_approve_or_approve() {
        ...
        let (choice, _) = MeetingOutcome::vote_from_monitoring(&m, &scout, 0.6);
        assert!(matches!(
            choice,
            ScoutVoteChoice::StrongApprove | ScoutVoteChoice::Approve
        ));
    ...
    fn poor_role_fit_with_low_adaptability_rejects() {
        ...
        let mut m = fresh_monitoring(1, 99);
        m.role_fit = 0.7;
        let (choice, reason) = MeetingOutcome::vote_from_monitoring(&m, &scout, 0.6);
    ...
    fn over_budget_rejects() {
        let scout = baseline_scout(1);
        let m = fresh_monitoring(1, 99);
        let (choice, reason) = MeetingOutcome::vote_from_monitoring(&m, &scout, 2.5);
        assert_eq!(choice, ScoutVoteChoice::Reject);
