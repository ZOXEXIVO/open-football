...
impl PositionDepth {
    /// The club's main-team depth in `group`, or `None` when it has no
    /// main team.
    fn at(club: &Club, group: PlayerFieldPositionGroup) -> Option<PositionDepth> {
    ...
    /// actually needs depth at this position. Surplus positions get scaled
    /// down; clear gaps get a small boost.
    fn weight_for(&self, candidate_ability: u8) -> f32 {
        if self.count == 0 {
            return 1.25;
        }
        if self.count >= self.max {
            // Surplus — only candidates clearly better than our best should
            // score well.
            let gap = candidate_ability as i16 - self.best_ability as i16;
            if gap >= 10 {
                0.95
            } else if gap >= 0 {
                0.7
            ...
            let fill = self.count as f32 / self.max as f32;
            1.0 + (1.0 - fill) * 0.2
        }
    }
}

/// What the board is told about a player it has never seen. Folded once per
/// country, because a proposal has to be weighed against numbers that are
/// not on the shortlist row.
#[derive(Clone, Copy)]
...
/// One board hearing's verdict, staged so the whole read pass can finish
/// before anything mutates a club.
struct Decision {
    club_id: u32,
    request_id: u32,
    approved: bool,
...
/// The club-level numbers every proposal this club puts to its board is
/// weighed against — read once per club rather than once per shortlist.
#[derive(Clone, Copy)]
struct BoardClubFinances {
    remaining_transfer_budget: f64,
...
impl ShortlistPass {
    ...
    fn build_club_shortlists(
        ...
        let mut results: Vec<ShortlistResult> = Vec::new();
        {
            ...
            Self::shortlists_from_scouting(
                ...
                plan,
                &buyer_ctx,
                &existing_shortlist_request_ids,
                &mut results,
            );

            Self::shortlists_from_market(
                country,
                club,
                player_lookup,
                date,
    ...
    pub fn evaluate_board_approvals(country: &mut Country, date: NaiveDate) {
        ...
        Self::apply_board_decisions(country, decisions);

        Self::leak_approved_pursuits(country, date, approved_targets);
    }

    /// One shortlist per scouting assignment that has produced a report.
    #[allow(clippy::too_many_arguments)]
    fn shortlists_from_scouting(
        country: &Country,
        club: &Club,
        player_lookup: &CountryPlayerLookup,
        date: NaiveDate,
        ...
        existing_shortlist_request_ids: &[u32],
        results: &mut Vec<ShortlistResult>,
    ) {
        ...
        for assignment in completed_assignments {
            ...
            let candidates = Self::score_reported_candidates(
                country,
                club,
                player_lookup,
                date,
    ...
    /// Market shortlist: for any request without one, build it directly from
    /// market listings. This keeps transfers flowing while scouts work on
    /// deeper evaluations in parallel.
    #[allow(clippy::too_many_arguments)]
    fn shortlists_from_market(
        country: &Country,
        club: &Club,
        player_lookup: &CountryPlayerLookup,
        date: NaiveDate,
        ...
        for request in &plan.transfer_requests {
            ...
            if !market_candidates.is_empty() {
                let mut shortlist = TransferShortlist::new(request.id, request.budget_allocation);
    ...
    /// Grade one assignment's reports into a ranked ten. Everything that can
    /// veto a name outright — surplus on arrival, unregistrable, implausible —
    /// runs before the scoring; everything that only shades the club's
    /// enthusiasm is a multiplier on top of the base read.
    #[allow(clippy::too_many_arguments)]
    fn score_reported_candidates(
        country: &Country,
        club: &Club,
        player_lookup: &CountryPlayerLookup,
        date: NaiveDate,
        ...
        let mut candidates: Vec<ShortlistCandidate> = reports
            ...
            .filter_map(|r| {
                ...
                if fit.would_be_surplus(r.assessed_ability, r.assessed_potential, candidate_age) {
                    return None;
                }
            ...
            .map(|(r, plausibility_mult)| {
                ...
                let depth_mult = match &depth {
                    Some(d) => d.weight_for(r.assessed_ability),
                    None => 1.0,
                };
    ...
    /// One walk over every roster in the country, so a board hearing does not
    /// have to go looking for the man it is being asked to buy.
    fn snapshot_players(
        country: &Country,
        date: NaiveDate,
    ...
    /// Every club's open shortlists, put in front of its own board. Read-only:
    /// each hearing is staged as a [`Decision`] and applied afterwards.
    fn review_shortlists(
        ...
        for club in &country.clubs {
            ...
            let finances = BoardClubFinances {
                remaining_transfer_budget: club
                    .finance
                    .transfer_budget
                    .as_ref()
                    .map(|b| b.amount)
                    .unwrap_or(plan.total_budget),
                squad_avg_ability: club
                    .teams
                    .main()
    ...
    /// Commit the hearings: pin the named target or move the club down its
    /// own list, drift the chairman-manager bond, and tell the scout who took
    /// the dossier in how it went.
    fn apply_board_decisions(country: &mut Country, decisions: Vec<Decision>) {
        for d in decisions {
    ...
    /// The rumour mill: a board-approved pursuit leaks.
    fn leak_approved_pursuits(
        country: &mut Country,
        date: NaiveDate,
        approved_targets: Vec<(u32, u32)>,
    ) {
    ...
    /// What the board is actually handed: the scouting dossier behind the
    /// name, the economics of paying for him, and the proposal itself.
    ...
    fn board_proposal(
        ...
        let wage_budget = finances.wage_budget;
        let league_country_id = finances.league_country_id;
