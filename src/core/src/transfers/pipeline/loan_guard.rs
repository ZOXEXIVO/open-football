...
use crate::transfers::pipeline::plausibility::{EffectivePlayerReputation, thresholds};
use crate::{
...
impl LoanAssetGuard {
    ...
    pub fn assess(&self, borrower: &LoanBorrowerProfile) -> LoanGuardVerdict {
        ...
        let capacity_penalty = (((weight - Self::W_SOFT) / (Self::W_MAX - Self::W_SOFT)) as f32)
            .clamp(0.0, 1.0);
...
impl LoanGuardVerdict {
    ...
    /// The money half on its own — used by the plausibility fee gate,
    /// which blocks an out-of-reach asset at public-interest stage rather
    /// than silently.
    pub fn beyond_borrower_means(&self) -> bool {
        self.weight > LoanAssetGuard::W_MAX || self.carry > LoanAssetGuard::CARRY_MAX
    }
}
