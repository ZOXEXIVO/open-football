...
pub async fn europa_league_get_action(
    ...
    let el = simulator_data
        .continents
        .iter()
        .find(|c| c.name == "Europe")
        .map(|c| &c.continental_competitions.europa_league)
        .filter(|el| {
            !el.groups.is_empty() || !matches!(el.current_stage, CompetitionStage::NotStarted)
        });
    ...
    if let Some(el) = el {
        current_stage = el.current_stage.as_i18n_key();

        for (idx, group) in el.groups.iter().enumerate() {
