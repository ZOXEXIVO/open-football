...
// Aliased: this module defines its own `PlayerStatistics` DTO, and the
...
pub async fn player_get_action(
    ...
    if let Some(team) = team_opt {
        ...
        let player_vm = PlayerViewModel {
            ...
            preferred_foot: i18n.t(player.preferred_foot.as_i18n_key()).to_string(),
...
impl<'a> PlayerOverviewStatsBuilder<'a> {
    ...
    /// label otherwise.
    ///
    /// The rating column is the plain arithmetic mean over rated
    /// appearances — FM's `Av Rat` — via
    /// [`PlayerStatistics::display_average_rating`]. Sample-size
    /// regression is a sort key and a decision currency, never a printed
    /// number: this panel sits on the same page as the player's match
    /// list, so an average that doesn't reconcile against it is a bug no
    /// matter how well-calibrated the shrinkage is.
    fn build(&self, player: &Player, team: Option<&Team>) -> Vec<CompetitionStatisticsRow> {
        ...
        let live_cups: Vec<LiveCupSlice<'_>> = player
            ...
            .collect();
        // Same source resolution as the History page, so a youth-squad
        // player's live friendly slice carries its real league slug
        // ("premier-league-u19") and the row below can wear that name.
        let friendly_source_slug = FriendlySourceSlug::resolve(player, team, self.data);
    ...
    /// projection stamps the row's `competition_slug` when every merged
    /// entry came from ONE league ("first-league",
    /// "premier-league-u19"); resolve it to the live league name, an
    /// i18n entry, or the ledger-frozen name. An empty slug — mixed
    /// sources, or a friendly with no specific source league — falls
    /// back to the generic kind label.
    fn league_row_label(&self, slug: &str, ledger_name: &str, generic_key: &str) -> String {
        if !slug.is_empty() {
            ...
            if !ledger_name.is_empty() {
                return ledger_name.to_string();
            }
        }
        self.i18n.t(generic_key).to_string()
    }
    ...
    fn to_dto(s: &SeasonStatistics) -> PlayerStatistics {
        PlayerStatistics {
            played: s.played,
            ...
            tackling: s.tackling,
            passes: s.passes,
            average_rating: s.display_average_rating(),
            conceded: s.conceded,
            clean_sheets: s.clean_sheets,
        }
    }

    fn empty_dto() -> PlayerStatistics {
        Self::to_dto(&SeasonStatistics::default())
    }
}
